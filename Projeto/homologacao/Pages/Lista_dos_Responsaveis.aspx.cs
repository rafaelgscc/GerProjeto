...
{
	public partial class Lista_dos_Responsaveis : GeneralDataPage
	{
		protected Lista_dos_ResponsaveisPageProvider PageProvider;
		...
		public override string PageName { get { return "Lista_dos_Responsaveis.aspx"; } }
		...
		{
			PageProvider = new Lista_dos_ResponsaveisPageProvider(this);
		}
		...
		public override GeneralDataProviderItem LoadItemFromGridControl(bool EnableValidation, string GridId)
		{
			...
			{
				case "Grid1":
					if (PageProvider.Lista_dos_Responsaveis_Grid1Provider.DataProvider.Item == null)
						Item = PageProvider.Lista_dos_Responsaveis_Grid1Provider.GetDataProviderItem();
					else
						Item = PageProvider.Lista_dos_Responsaveis_Grid1Provider.DataProvider.Item;
					PageProvider.Lista_dos_Responsaveis_Grid1Provider.RaiseSetRelationFields(PageProvider.Lista_dos_Responsaveis_Grid1Provider, Item);
					Item.SetFieldValue(Item["siglaDiretoria"], PageProvider.Lista_dos_Responsaveis_Grid1Provider.GridData["siglaDiretoria"]);
					Item.SetFieldValue(Item["siglaCoordenacao"], PageProvider.Lista_dos_Responsaveis_Grid1Provider.GridData["siglaCoordenacao"]);
					Item.SetFieldValue(Item["nomeResponsavel"], PageProvider.Lista_dos_Responsaveis_Grid1Provider.GridData["nomeResponsavel"]);
					Item.SetFieldValue(Item["ramalResponsavel"], PageProvider.Lista_dos_Responsaveis_Grid1Provider.GridData["ramalResponsavel"]);
					Item.SetFieldValue(Item["contatoResponsavel"], PageProvider.Lista_dos_Responsaveis_Grid1Provider.GridData["contatoResponsavel"]);
					Item.SetFieldValue(Item["salaResponsavel"], PageProvider.Lista_dos_Responsaveis_Grid1Provider.GridData["salaResponsavel"]);
					PageProvider.Lista_dos_Responsaveis_Grid1Provider.InitializeAlias(Item);
					if (EnableValidation)
					{
						PageProvider.Lista_dos_Responsaveis_Grid1Provider.Validate(Item);
					}
					break;
			}
		...
		protected void Grid1_ItemCreated(object sender, GridItemEventArgs e)
		{
			if (e.Item is GridEditableItem && (e.Item.IsInEditMode))
			{
				...
				txt.Attributes.Add("data-validation-message", "Sigla da Diretoria não pode ser vazio!");
				...
				txt.Width = 128;
				Mask.SetMask(txt, "(99) 99999-9999", 15, false);
				txt.Attributes.Add("onblur", "OnMaskBlur();");
				txt.Attributes.Add("isGrid", "true");
				ApplyMasks(txt);
			...
			else if(e.Item is GridFilteringItem)
			{
				...
				FilteringItem["GridColumn10"].CssClass = "hasFilterButton";
				FilteringItem["GridColumn11"].CssClass = "hasFilterButton";
				FilteringItem["GridColumn2"].CssClass = "hasFilterButton";
				FilteringItem["GridColumn4"].CssClass = "hasFilterButton";
				FilteringItem["GridColumn5"].CssClass = "hasFilterButton";
				FilteringItem["GridColumn6"].CssClass = "hasFilterButton";
				FilteringItem["GridColumn9"].CssClass = "hasFilterButton";
			}
		}
		...
		protected void Grid1_ItemCommand(object source, GridCommandEventArgs e)
		{
			...
			if (e.Item is GridDataItem && !(e.Item is GridDataInsertItem) && e.CommandName != "Cancel" && e.CommandName != "Update" && e.CommandName != "Insert")
			{
				...
				switch (e.CommandName)
				{
					case "GridColumn9":
						...
						try
						{
							string UrlPage = ResolveUrl("~/Pages/TB_RESPONSAVEL.aspx?ParamCodigo_Pes={ParamCodigo_Pes}");
							UrlPage = UrlPage.Replace("{ParamCodigo_Pes}", (Convert.ToInt32(Provider.DataProvider.Item["codigo"].GetValue())).ToString());
							try
							{
			...
			if (e.CommandName == RadGrid.FilterCommandName)
			{
				...
				Grid.MasterTableView.Columns.FindByUniqueName(filterPair.Second.ToString()).CurrentFilterFunction = ((GridKnownFunction)System.Enum.Parse(typeof(GridKnownFunction), filterPair.First.ToString()));
			}
		}
		protected FieldType ReplaceDataType(string dataType)
		{
			switch (dataType)
			{
				case "Binary":
		...
		protected void Grid_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
		{
			...
			{
				string field;
				switch (column.CurrentFilterFunction.ToString())
				{
					case "Contains":
						if (!String.IsNullOrEmpty(GridFilter)) GridFilter += " AND ";
						field = ((GridEditableColumn)column).GetActiveDataField();
						GridFilter += Dao.PoeColAspas(field) + " LIKE " + Dao.ToSql("%" + column.CurrentFilterValue + "%", ReplaceDataType(((GridEditableColumn)column).DataType.Name));
						break;
					case "DoesNotContain":
						if (!String.IsNullOrEmpty(GridFilter)) GridFilter += " AND ";
						field = ((GridEditableColumn)column).GetActiveDataField();
						GridFilter += Dao.PoeColAspas(field) + " NOT LIKE " + Dao.ToSql("%" + column.CurrentFilterValue + "%", ReplaceDataType(((GridEditableColumn)column).DataType.Name));
						break;
					case "StartsWith":
						if (!String.IsNullOrEmpty(GridFilter)) GridFilter += " AND ";
						field = ((GridEditableColumn)column).GetActiveDataField();
					...
					case "EqualTo":
						if (!String.IsNullOrEmpty(GridFilter)) GridFilter += " AND ";
						field = ((GridEditableColumn)column).GetActiveDataField();
						GridFilter += Dao.PoeColAspas(field) + " = " + Dao.ToSql(column.CurrentFilterValue, ReplaceDataType(((GridEditableColumn)column).DataType.Name));
						break;
					case "NotEqualTo":
						if (!String.IsNullOrEmpty(GridFilter)) GridFilter += " AND ";
						field = ((GridEditableColumn)column).GetActiveDataField();
						GridFilter += "Not " + field + " = " + Dao.ToSql(column.CurrentFilterValue, ReplaceDataType(((GridEditableColumn)column).DataType.Name));
						break;
					case "GreaterThan":
						if (!String.IsNullOrEmpty(GridFilter)) GridFilter += " AND ";
						field = ((GridEditableColumn)column).GetActiveDataField();
						GridFilter += field + " > " + Dao.ToSql(column.CurrentFilterValue, ReplaceDataType(((GridEditableColumn)column).DataType.Name));
						break;
					case "LessThan":
						if (!String.IsNullOrEmpty(GridFilter)) GridFilter += " AND ";
						field = ((GridEditableColumn)column).GetActiveDataField();
					...
					case "IsNull":
						if (!String.IsNullOrEmpty(GridFilter)) GridFilter += " AND ";
						field = ((GridEditableColumn)column).GetActiveDataField();
						GridFilter += field + " IS NULL ";
						break;
					case "NotIsNull":
						if (!String.IsNullOrEmpty(GridFilter)) GridFilter += " AND ";
						field = ((GridEditableColumn)column).GetActiveDataField();
						GridFilter += field + " IS NOT NULL ";
						break;
				}
			}
			if (GridFilter != "")
			{
				...
				GetGridProvider(Grid).DataProvider.InitialFilter = GridFilter;
			}
		...
		private void InitializeGridData(RadGrid Grid)
		{
			...
			{
				case "Grid1":
					GetGridProvider(Grid).DataProvider.OrderBy = "[siglaDiretoria] Asc, [siglaCoordenacao] Asc, [nomeResponsavel] Asc";
					break;
			}
		}
		...
		public override GeneralGridProvider GetGridProvider(RadGrid Grid)
		{
			...
			{
				case "Grid1":
					return PageProvider.Lista_dos_Responsaveis_Grid1Provider;
			}
			return null;
		}
	}
