...
{
	public partial class EditarUsuario : GeneralDataPage
	{
		...
		public override void UpdateItemFromControl(GeneralDataProviderItem  Item)
		{
			...
			if (PageState != FormStateEnum.Navigation && this.IsPostBack)
			{
				...
				Item.SetFieldValue(Item["LOGIN_USER_PHONE"], RadTextBox6.Text, false);
		...
		public override void DefinePageContent(GeneralDataProviderItem Item)
		{
			...
			try
			{
				if (Item != null)
				{
					RadTextBox6.Text = Item["LOGIN_USER_PHONE"].GetFormattedValue();
				}
				else
				{
					RadTextBox6.Text = "" ;
		...
		protected void ___btnPesquisar_OnClick(object sender, EventArgs e)
		{
			...
			try
			{
				...
				UrlPage = UrlPage.Replace("{ParamOrigem}", ("EditarUsuario").ToString());
				try
				{
