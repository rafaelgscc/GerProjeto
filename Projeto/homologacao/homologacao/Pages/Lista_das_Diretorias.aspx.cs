...
{
	public partial class Lista_das_Diretorias : GeneralDataPage
	{
		protected Lista_das_DiretoriasPageProvider PageProvider;
		...
		public override string PageName { get { return "Lista_das_Diretorias.aspx"; } }
		...
		{
			PageProvider = new Lista_das_DiretoriasPageProvider(this);
		}
		...
		public override GeneralDataProviderItem LoadItemFromGridControl(bool EnableValidation, string GridId)
		{
			...
			{
				case "Grid1":
					if (PageProvider.Lista_das_Diretorias_Grid1Provider.DataProvider.Item == null)
						Item = PageProvider.Lista_das_Diretorias_Grid1Provider.GetDataProviderItem();
					else
						Item = PageProvider.Lista_das_Diretorias_Grid1Provider.DataProvider.Item;
					PageProvider.Lista_das_Diretorias_Grid1Provider.RaiseSetRelationFields(PageProvider.Lista_das_Diretorias_Grid1Provider, Item);
					Item.SetFieldValue(Item["siglaDiretoria"], PageProvider.Lista_das_Diretorias_Grid1Provider.GridData["siglaDiretoria"]);
					Item.SetFieldValue(Item["nomeDiretoria"], PageProvider.Lista_das_Diretorias_Grid1Provider.GridData["nomeDiretoria"]);
					Item.SetFieldValue(Item["nomeDiretor"], PageProvider.Lista_das_Diretorias_Grid1Provider.GridData["nomeDiretor"]);
					PageProvider.Lista_das_Diretorias_Grid1Provider.InitializeAlias(Item);
					if (EnableValidation)
					{
						PageProvider.Lista_das_Diretorias_Grid1Provider.Validate(Item);
					}
					break;
			}
			...
			if (e.Item is GridDataItem && !(e.Item is GridDataInsertItem) && e.CommandName != "Cancel" && e.CommandName != "Update" && e.CommandName != "Insert")
			{
				...
				switch (e.CommandName)
				{
					case "GridColumn5":
						...
						try
						{
							string UrlPage = ResolveUrl("~/Pages/CadastroDiretorias.aspx?Param_Codigo_Dir={Param_Codigo_Dir}");
							UrlPage = UrlPage.Replace("{Param_Codigo_Dir}", (Convert.ToInt32(Provider.DataProvider.Item["codigo"].GetValue())).ToString());
							try
							{
		...
		public override GeneralGridProvider GetGridProvider(RadGrid Grid)
		{
			...
			{
				case "Grid1":
					return PageProvider.Lista_das_Diretorias_Grid1Provider;
			}
			return null;
		}
	}
