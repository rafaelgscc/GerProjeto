...
{
	public partial class Coordenacao_Geral : GeneralDataPage
	{
		protected Coordenacao_GeralPageProvider PageProvider;
		...
		public override string PageName { get { return "Coordenacao_Geral.aspx"; } }
		...
		{
			PageProvider = new Coordenacao_GeralPageProvider(this);
		}
		...
		public override GeneralDataProviderItem LoadItemFromGridControl(bool EnableValidation, string GridId)
		{
			...
			{
				case "Grid1":
					if (PageProvider.Lista_das_Coordenacoes_Grid1Provider.DataProvider.Item == null)
						Item = PageProvider.Lista_das_Coordenacoes_Grid1Provider.GetDataProviderItem();
					else
						Item = PageProvider.Lista_das_Coordenacoes_Grid1Provider.DataProvider.Item;
					PageProvider.Lista_das_Coordenacoes_Grid1Provider.RaiseSetRelationFields(PageProvider.Lista_das_Coordenacoes_Grid1Provider, Item);
					Item.SetFieldValue(Item["siglaDiretoria"], PageProvider.Lista_das_Coordenacoes_Grid1Provider.GridData["siglaDiretoria"]);
					Item.SetFieldValue(Item["siglaCoordenacao"], PageProvider.Lista_das_Coordenacoes_Grid1Provider.GridData["siglaCoordenacao"]);
					Item.SetFieldValue(Item["nomeCoordenacao"], PageProvider.Lista_das_Coordenacoes_Grid1Provider.GridData["nomeCoordenacao"]);
					Item.SetFieldValue(Item["nomeCoordenador"], PageProvider.Lista_das_Coordenacoes_Grid1Provider.GridData["nomeCoordenador"]);
					PageProvider.Lista_das_Coordenacoes_Grid1Provider.InitializeAlias(Item);
					if (EnableValidation)
					{
						PageProvider.Lista_das_Coordenacoes_Grid1Provider.Validate(Item);
					}
					break;
			}
			...
			if (e.Item is GridDataItem && !(e.Item is GridDataInsertItem) && e.CommandName != "Cancel" && e.CommandName != "Update" && e.CommandName != "Insert")
			{
				...
				switch (e.CommandName)
				{
					case "GridColumn6":
						...
						try
						{
							string UrlPage = ResolveUrl("~/Pages/CadastroCoordenacoesGerais.aspx?ParamCodigo_Coord={ParamCodigo_Coord}");
							UrlPage = UrlPage.Replace("{ParamCodigo_Coord}", (Convert.ToInt32(Provider.DataProvider.Item["codigo"].GetValue())).ToString());
							try
							{
		...
		public override GeneralGridProvider GetGridProvider(RadGrid Grid)
		{
			...
			{
				case "Grid1":
					return PageProvider.Lista_das_Coordenacoes_Grid1Provider;
			}
			return null;
		}
	}
