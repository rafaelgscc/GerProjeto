...
namespace PROJETO.DataPages
{
	...
	public abstract class GeneralDataPage : Page, IGeneralDataProvider
	{
		...
		public void ApplyMasks(WebControl Txt)
		{
			...
			{
				string script = "ApplyElementMask(document.getElementById('" + Txt.ClientID + "'));";
				if (!IsPostBack)
				{
					ClientScript.RegisterStartupScript(this.GetType(), "Mask" + Txt.ClientID, "<script>" + script + "</script>");
				}
				else
				{
					if (AjaxPanel.ResponseScripts.Contains(script)) AjaxPanel.ResponseScripts.Remove(script);
