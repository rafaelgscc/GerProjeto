...
namespace PROJETO.DataProviders
{
	...
	/// </summary>
	public class CadastroCoordenacoesSetoriaisPageProvider : GeneralProvider
	{
		...
		public CadastroCoordenacoesSetoriaisPageProvider(IGeneralDataProvider Provider)
		{
			MainProvider = Provider;
			MainProvider.DataProvider = new DBGERPROJETO_TB_SETORIALDataProvider(MainProvider, MainProvider.TableName, MainProvider.DatabaseName, "PK_TB_SETORIAL", "CadastroCoordenacoesSetoriais");
			MainProvider.DataProvider.PageProvider = this;
			ComboBox1Provider = new DBGERPROJETO_TB_DIRETORIADataProvider(MainProvider, "TB_DIRETORIA", "DBGERPROJETO", "", "CadastroCoordenacoesSetoriais_ComboBox1ProviderAlias");
			ComboBox1Provider.PageProvider = this;
			ComboBox1Provider.CreatingParameters += GeneralDataProvider.GeneralCreatingParameters;
			ComboBox2Provider = new DBGERPROJETO_TB_COORDENACAODataProvider(MainProvider, "TB_COORDENACAO", "DBGERPROJETO", "", "CadastroCoordenacoesSetoriais_ComboBox2ProviderAlias");
		...
		public override GeneralDataProviderItem GetDataProviderItem(GeneralDataProvider Provider)
		{
			...
			{ 
				return new DBGERPROJETO_TB_SETORIALItem(MainProvider.DatabaseName);
			}
		...
		public override bool Validate(GeneralDataProviderItem ProviderItem)
		{
			...
			if (!Accepted) { ProviderItem.Errors.Add("ServerValidationError:ComboBox2", "Sigla da Coordenacao não pode ser vazio!");}
			try
			{
				Accepted =(ServerValidation.CheckNotEmpty(AliasVariables["siglaSetorialField"]));
			}
			catch (Exception)
			{
				Accepted = false;
			}
			if (!Accepted) { ProviderItem.Errors.Add("ServerValidationError:RadTextBox4", "Sigla do Setor não pode ser vazio!");}
			try
			{
				Accepted =(ServerValidation.CheckNotEmpty(AliasVariables["nomeSetorialField"]));
			}
			catch (Exception)
			{
				Accepted = false;
			}
			if (!Accepted) { ProviderItem.Errors.Add("ServerValidationError:RadTextBox5", "Nome do Setor não pode ser vazio!");}
			try
			{
